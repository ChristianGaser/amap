...
//-----------------------------------------------------------------------
// Constructor
TBSpline::TBSpline(const Mat<double> &domain, 
...
//
void 
TBSpline::bendingEnergyTensor(const IntArray &n, Mat<double> &J)
{
  ...
  Mat<double> (*bendingMatrix)[3] = new Mat<double>[nDimensions][3];  
  for(i = 0; i < nDimensions; i++)
    for(j = 0; j < 3; j++)
...
TBSpline::bendingEnergy(int size, int order)
{
  ...
  Mat<double> integral(6,4);  // regions by offset
...
Mat<double>
TBSpline::solveSymmetricSystem(Mat<double> &A, Mat<double> b, int *info)
{
...
TBSpline::test_J(void)
{
  ...
  for(int j = 0; j < 3; j++)
    for(int i = 4; i < 8; i++)
      {
  Mat<double> J = bendingEnergy(i, j);
...
TBSpline::test_solver(void)
{
  ...
  Mat<double> x = solveSymmetricSystem(A,b,&info);
...
//-----------------------------------------------------------------------
// Constructors
TBSplineVolume::TBSplineVolume(const Mat<double> &domain, 
                    // an n by 2 matrix specifying a region in R^n
                    // upon which the spline is defined
                    // eg. [ x1 x2; y1 y2 ] 
...
Mat<double> TBSplineVolume::computeDomain(const double start[VDIM],
                                     const double step[VDIM],
                                     const int count[VDIM])
{
  Mat<double> domain(VDIM,2);
