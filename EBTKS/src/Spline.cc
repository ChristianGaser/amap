...
TPSpline::clearDataPoints()
{
  ...
  _Ft = Mat<double>(1, _nDimensions + 1);
  _Aj = Mat<double>(1, _nKnots);
...
{
  // Calculate C
  Mat<double> R(Ones<double>(_nDimensions + 1, _nKnots - _nDimensions - 1));
  for (unsigned spline = _nDimensions + 1, i = 0; spline < _nKnots; spline++, i++)
    for (unsigned d = 0; d < _nDimensions; d++)
      R(d + 1, i) = _knots(spline, d);

  Mat<double> J = _J.cols(0, _nDimensions) * _Psi * R + _J.cols(_nDimensions + 1, _nKnots-1);
  J = R.t() * _Psi.t() * J.rows(0, _nDimensions) + J.rows(_nDimensions + 1, _nKnots-1);
  J.pad(_nKnots, _nKnots, 0, 0);

  // Calculate W
  Mat<double> W(inv(_AtA + (_lambda*_nsamples)*J) * _AtF);
