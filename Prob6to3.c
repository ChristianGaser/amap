...
unsigned char * Prob6to3(unsigned char *prob, unsigned char *label0, double *mean, int BG, int *dims)
{
  int x,y,z,i,z_area,y_dims,ind,mxi;
  double val[6],w[5],*out,psum,mx,tmp_val[3];
  ...
  w[0] = mean[0]/mean[1];
  w[1] = 1 - w[0];
  w[2] = (mean[2]-mean[1])/(mean[3]-mean[1]);
  w[3] = 1 - w[2];
  w[4] = (mean[4]-mean[3])/(mean[5]-mean[3]);
  w[5] = 1 - w[4];
  ...
  for (z = 1; z < dims[2]-1; z++) {
    ...
    for (y = 1; y < dims[1]-1; y++) {
      ...
      for (x = 1; x < dims[0]-1; x++) {
        ind = z_area + y_dims + x;
        // get original 6 probabilities
        for (i = 0; i < 6; i++)
          val[i] = (double)prob[(i*vol) + ind];

        if (label0[ind] >= BG) {
        // calculate 3 new probabilities using weight
          tmp_val[0] = val[1] + (w[0]*val[0]) + (w[3]*val[2]);
          tmp_val[1] = val[3] + (w[2]*val[2]) + (w[5]*val[5]);
          tmp_val[2] = val[5] + (w[4]*val[4]);
