...
{
  int i, j, k, l, m, k1, subit, subsample_warp, kmax, k2;
  int count, subsample, masked_smoothing, correct_nu;
...
//  for (j=0; j < 3; j++) {
  for (j=0; j < 2; j++) {
    ...
    /* decrease subsample with each iteration */
//    if ((j > 0) && (subsample_warp > 1))  subsample_warp -= 1;
...
if (0) {
      ...
      if ((correct_nu) && (j > 0)){
        ...
        masked_smoothing = 0;
          ...
          if (subsample == 1) {
          smooth_float(meaninvcov, dims, separations, fwhm, masked_smoothing);
          smooth_float(resmean, dims, separations, fwhm, masked_smoothing);
        } else {
