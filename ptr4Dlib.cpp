...
void ptr_to_float_4D2(nifti_image *image, float *vol4d, int index_nt)
{
	...
	for(unsigned int index=0; index<image->nvox; index++){
		...
		float value = (float)*imagePtr * image->scl_slope + image->scl_inter;
	    vol4d[index+(int)index_nt*(int)image->nvox] = value;
...
void
ptr_to_float_4D(nifti_image *image, float *vol4d, int index_nt) 
{

	switch(image->datatype){
		case NIFTI_TYPE_UINT8:
			ptr_to_float_4D2<unsigned char>(image, vol4d, index_nt);
			break;
		case NIFTI_TYPE_INT8:
			ptr_to_float_4D2<char>(image, vol4d, index_nt);
			break;
		case NIFTI_TYPE_UINT16:
			ptr_to_float_4D2<unsigned short>(image, vol4d, index_nt);
			break;
		case NIFTI_TYPE_INT16:
			ptr_to_float_4D2<short>(image, vol4d, index_nt);
			break;
		case NIFTI_TYPE_UINT32:
			ptr_to_float_4D2<unsigned int>(image, vol4d, index_nt);
			break;
		case NIFTI_TYPE_INT32:
			ptr_to_float_4D2<int>(image, vol4d, index_nt);
			break;
		case NIFTI_TYPE_FLOAT32:
			ptr_to_float_4D2<float>(image, vol4d, index_nt);
			break;
		case NIFTI_TYPE_FLOAT64:
			ptr_to_float_4D2<double>(image, vol4d, index_nt);
			break;
		default:
			printf("err\tptr_to_double_4D2\tThe image data type is not supported\n");
			return;
