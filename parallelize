...
parallelize ()
{
  ...
  do
    if [ ! "${ARG_LIST[$i]}" == "" ]; then
      j=$(($i+1))
      echo job ${j}/"$NUMBER_OF_JOBS": $COMMAND ${ARG_LIST[$i]}
      echo job ${j}/"$NUMBER_OF_JOBS": $COMMAND ${ARG_LIST[$i]} >> $log
      nohup $COMMAND ${ARG_LIST[$i]}>> $log 2>&1 &
    fi
    ((i++))
  done
...
USAGE:
  parallize.sh [-p number_of_processes] -c command_to_parallelize filename|filepattern
...
OUTPUT:
   parallelize_${time}.log with current data and time in name as log-file

EXAMPLE
   parallelize.sh -c "niismooth -fwhm 10" sTRIO*.nii
   Parallelize smoothing with fwhm of 10mm for all files sTRIO*.nii. 
   
   parallelize.sh -c gunzip *.zip
