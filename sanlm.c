...
void Average_block(double *ima,int x,int y,int z,int neighborhoodsize,float *average, double weight, int sx,int sy,int sz)
{
...
sxy = sx*sy;
    ...
    for (c = 0; c<ns;c++)
    {
        ...
        {
                  if (is_outside)
                       average[index] += (float)(ima[z*(sxy)+(y*sx)+x]*ima[z*(sxy)+(y*sx)+x]*weight);
                  else average[index] += (float)(ima[z_pos*(sxy)+(y_pos*sx)+x_pos]*ima[z_pos*(sxy)+(y_pos*sx)+x_pos]*weight);
        } 
        else
        {
          if (is_outside)
                       average[index] += (float)(ima[z*(sxy)+(y*sx)+x]*weight);
                  else average[index] += (float)(ima[z_pos*(sxy)+(y_pos*sx)+x_pos]*weight);
        }
            }
        }
    }
...
for (k = -f; k <= f; k++)
{
  ...
  for (j = -f; j <= f; j++)
  {
    ...
    for (i = -f; i<= f; i++)
    {
      ...
      d = (ima[nk1*(sx*sy)+(nj1*sx)+ni1]-ima[nk2*(sx*sy)+(nj2*sx)+ni2]);
      distancetotal += d*d;   
    }
  }
}
...
double distance2(double *ima,float *medias,int x,int y,int z,int nx,int ny,int nz,int f,int sx,int sy,int sz)
{
double d,acu,distancetotal;
int i,j,k,ni1,nj1,ni2,nj2,nk1,nk2;
...
for (k = -f; k <= f; k++)
{
  ...
  for (j = -f; j <= f; j++)
  {
    ...
    for (i = -f; i <= f; i++)
    {
      ...
      d = (ima[nk1*(sx*sy)+(nj1*sx)+ni1]-(double)medias[nk1*(sx*sy)+(nj1*sx)+ni1])-(ima[nk2*(sx*sy)+(nj2*sx)+ni2]-(double)medias[nk2*(sx*sy)+(nj2*sx)+ni2]);            
        distancetotal += d*d;              
    }
  }
}
...
{
float *bias,*Estimate,*means,*variances,*average;
double *ima,epsilon,mu1,var1,totalweight,wmax,t1,t1i,t2,d,w,distanciaminima;
unsigned char *Label;
            ...
            for (ii = -1;ii<= 1;ii++)
            {
                for (jj = -1;jj<= 1;jj++)
                {
                    for (kk = -1;kk<= 1;kk++)
                    {
                        ...
                        mean += ima[nk*(dims[0]*dims[1])+(nj*dims[0])+ni];
                        indice++;                
                ...
                for (jj = -1;jj<= 1;jj++)
                {
                    for (kk = -1;kk<= 1;kk++)
                    {
                        ...
                        if (ni>= 0 && nj>= 0 && nk>0 && ni<dims[0] && nj<dims[1] && nk<dims[2])
                            {
                            d = ima[nk*(dims[0]*dims[1])+(nj*dims[0])+ni]-(double)means[k*(dims[0]*dims[1])+(j*dims[0])+i];
                            var += d*d;
                            indice++;
                        }
                    }
